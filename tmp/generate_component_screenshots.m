...
function fig = shotNanoMotor(~)
% Disconnected, which is how the panel always opens: the constructor makes the
% driver and touches no serial port until Connect is pressed. There is no
% connected variant to capture -- IsConnected is a live serialport handle, so
% showing one would need a board on the bench.
fig = shotFigure([300 152]);
m = gui.components.NanoMotor([], fig, PreferenceTag='wikiShotNanoMotor');
fig.UserData = {m};
end
...
groups = {'epsych2_gui_History', 'epsych2_gui_NextTrial', ...
    ...
    'epsych2_gui_Notes', 'epsych2_gui_BufferPlot', 'epsych2_gui_NanoMotor', ...
